...
class UriToClashConverter:
    ...
    # ... (All other parsers like parse_vless_trojan, parse_ss, etc. remain unchanged) ...
    @staticmethod
    ...
    def parse_wireguard(uri: str) -> Dict:
        """
        MODIFIED: This function is now enhanced to handle both IPv4/IPv6 addresses
        and properly decode the private key.
        """
        parsed_uri = urlparse(uri)
        params = UriToClashConverter._get_params(uri)
        
        # URL-decode the private key from the username part of the URI
        ...
        # Handle combined IPv4/IPv6 addresses
        addresses = params.get('address', ['172.16.0.2/32'])[0].split(',')
        for addr in addresses:
            addr = addr.strip()
            if ':' in addr: # This is likely an IPv6 address
                proxy['ipv6'] = addr
            elif '.' in addr: # This is likely an IPv4 address
                proxy['ip'] = addr
        
        if params.get('presharedKey'):
            proxy['pre-shared-key'] = params['presharedKey'][0]
        
        # Set a default mtu if not present, as it's often needed
