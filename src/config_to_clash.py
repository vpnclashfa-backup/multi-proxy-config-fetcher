...
class UriToClashConverter:
    ...
    def parse_ss(uri: str) -> Dict:
        ...
        if 'plugin' in params:
            ...
            if params.get('path'):
                proxy['plugin-opts']['path'] = params.get('path')[0]
            if params.get('tls', ['false'])[0] == 'true':
    ...
    def parse_ssr(uri: str) -> Dict:
        try:
            ...
            proxy = {
                ...
                "type": "ssr", "server": server, "port": int(port), "cipher": method,
                "password": password, "obfs": obfs, "protocol": protocol,
    ...
    def parse_hysteria(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "hysteria", "server": parsed_uri.hostname, "port": parsed_uri.port,
            "auth-str": parsed_uri.username, "up": params.get('up', ['50'])[0],
            "down": params.get('down', ['100'])[0], "protocol": params.get('protocol', [None])[0],
            "sni": params.get('sni', [parsed_uri.hostname])[0], "skip-cert-verify": True
        }
    ...
    def parse_hysteria2(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "hysteria2", "server": parsed_uri.hostname, "port": parsed_uri.port,
            "password": parsed_uri.username, "sni": params.get('sni', [parsed_uri.hostname])[0],
    ...
    def parse_tuic(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "tuic", "server": parsed_uri.hostname, "port": parsed_uri.port,
            "uuid": uuid, "password": password, "sni": params.get('sni', [parsed_uri.hostname])[0],
            "alpn": [params.get('alpn', ['h3'])[0]], "skip-cert-verify": True,
    ...
    def parse_snell(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "snell", "server": parsed_uri.hostname, "port": parsed_uri.port,
            "psk": parsed_uri.username, "version": params.get('version', ['3'])[0],
        }
        if params.get('obfs'):
    ...
    def parse_ssh(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "ssh", "server": parsed_uri.hostname, "port": parsed_uri.port or 22,
            "username": parsed_uri.username, "password": parsed_uri.password,
        }
        return {k: v for k, v in proxy.items() if v is not None}
    ...
    def parse_wireguard(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "private-key": unquote(parsed_uri.username),
            "public-key": params.get('publicKey', [''])[0],
            "ip": params.get('address', ['172.16.0.2/32'])[0], "udp": True
        }
    ...
    def parse_anytls(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "anytls", "server": parsed_uri.hostname, "port": parsed_uri.port,
            "password": parsed_uri.username, "client-fingerprint": params.get('fp', ['chrome'])[0],
    ...
    def parse_mieru(uri: str) -> Dict:
        ...
        proxy = {
            ...
            "type": "mieru", "server": parsed_uri.hostname, "port": parsed_uri.port,
            "username": parsed_uri.username, "password": params.get('password', [''])[0],
...
def replace_placeholders(data, proxy_names):
    if isinstance(data, dict):
        for key, value in data.items():
            data[key] = replace_placeholders(value, proxy_names)
...
def main():
    ...
    for template_file in template_files:
        ...
        # Create a deep copy for the combined file to avoid modifying the original template_data
        import copy
