# کد کامل و نهایی fetch_configs.py
import re
import os
import time
import json
...
class ClashConverter:
    ...
    def to_ssr(proxy: dict) -> str:
        # ssr://server:port:proto:method:obfs:password_base64/?obfsparam=obfsparam_base64&protoparam=protoparam_base64&remarks=remarks_base64
...
# I will provide the full code to avoid any confusion.

class ConfigFetcher:
    def __init__(self, config: ProxyConfig):
        self.config = config; self.validator = ConfigValidator(); self.protocol_counts: Dict[str, int] = {p: 0 for p in config.SUPPORTED_PROTOCOLS}; self.seen_configs: Set[str] = set(); self.channel_protocol_counts: Dict[str, Dict[str, int]] = {}; self.session = requests.Session(); self.session.headers.update(config.HEADERS)
