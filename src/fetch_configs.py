...
# The rest of the file (ConfigFetcher class, save_configs, main, etc.) remains unchanged.
class ConfigFetcher:
    def __init__(self, config: ProxyConfig): self.config = config; self.validator = ConfigValidator(); self.protocol_counts: Dict[str, int] = {p: 0 for p in config.SUPPORTED_PROTOCOLS}; self.seen_configs: Set[str] = set(); self.channel_protocol_counts: Dict[str, Dict[str, int]] = {}; self.session = requests.Session(); self.session.headers.update(config.HEADERS)
    def fetch_with_retry(self, url: str) -> Optional[requests.Response]:
        backoff = 1
        for attempt in range(self.config.MAX_RETRIES):
            try:
                response = self.session.get(url, timeout=self.config.REQUEST_TIMEOUT); response.raise_for_status(); return response
            except requests.RequestException as e:
                if attempt == self.config.MAX_RETRIES - 1: logger.error(f"Failed to fetch {url} after {self.config.MAX_RETRIES} attempts: {str(e)}"); return None
                wait_time = min(self.config.RETRY_DELAY * backoff, 60); logger.warning(f"Attempt {attempt + 1} for {url} failed, retrying in {wait_time}s: {str(e)}"); time.sleep(wait_time); backoff *= 2
        return None
    def fetch_configs_from_source(self, channel: ChannelConfig) -> List[str]:
        configs: List[str] = []; channel.metrics.total_configs = 0; channel.metrics.valid_configs = 0; channel.metrics.protocol_counts = {p: 0 for p in self.config.SUPPORTED_PROTOCOLS}; start_time = time.time(); response = self.fetch_with_retry(channel.url)
        ...
        if not is_yaml:
            if channel.is_telegram:
                soup = BeautifulSoup(content, 'html.parser'); messages = soup.find_all('div', class_='tgme_widget_message_text')
                for message in messages: found_configs = self.validator.split_configs(message.get_text(separator='\n')); configs.extend(found_configs)
            else:
        ...
        unique_configs = list(dict.fromkeys(configs)); channel.metrics.total_configs = len(unique_configs); valid_configs = []
        ...
        if len(valid_configs) >= self.config.MIN_CONFIGS_PER_CHANNEL: self.config.update_channel_stats(channel, True, response_time)
        ...
        for protocol in self.config.SUPPORTED_PROTOCOLS:
            if config.startswith(protocol) and self.config.is_protocol_enabled(protocol):
                if self.validator.validate_protocol_config(config, protocol):
                    ...
                    channel.metrics.valid_configs += 1; channel.metrics.protocol_counts[protocol] = channel.metrics.protocol_counts.get(protocol, 0) + 1
                    if clean_config not in self.seen_configs: self.seen_configs.add(clean_config); processed_configs.append(clean_config); self.protocol_counts[protocol] = self.protocol_counts.get(protocol, 0) + 1
                break
        return processed_configs
    def extract_date_from_message(self, message) -> Optional[datetime]:
        try:
            time_element = message.find_parent('div', class_='tgme_widget_message').find('time');
            if time_element and 'datetime' in time_element.attrs: return datetime.fromisoformat(time_element['datetime'].replace('Z', '+00:00'))
        except: return None
    def is_config_valid(self, config_text: str, date: Optional[datetime]) -> bool:
        if not date: return True
        cutoff_date = datetime.now(timezone.utc) - timedelta(days=self.config.MAX_CONFIG_AGE_DAYS); return date >= cutoff_date
    def balance_protocols(self, configs: List[str]) -> Dict[str, List[str]]:
        ...
        for config in configs:
            ...
            for protocol in self.config.SUPPORTED_PROTOCOLS:
                if normalized_config.startswith(protocol): protocol_configs[protocol].append(config); break
    ...
    def fetch_all_configs(self) -> Dict[str, List[str]]:
        ...
        for idx, channel in enumerate(enabled_channels, 1):
            ...
            try: channel_configs = self.fetch_configs_from_source(channel); all_configs.extend(channel_configs)
            except Exception as e: logger.error(f"Failed to fetch or process {channel.url}: {e}")
            if idx < len(enabled_channels): time.sleep(2)
        unique_configs = sorted(list(dict.fromkeys(all_configs))); return self.balance_protocols(unique_configs)
...
def save_channel_stats(config: ProxyConfig):
    try:
        ...
        for channel in config.SOURCE_URLS:
            channel_stats = {'url': channel.url, 'enabled': channel.enabled, 'metrics': {'total_configs': channel.metrics.total_configs, 'valid_configs': channel.metrics.valid_configs, 'unique_configs': channel.metrics.unique_configs, 'avg_response_time': round(channel.metrics.avg_response_time, 2), 'success_count': channel.metrics.success_count, 'fail_count': channel.metrics.fail_count, 'overall_score': round(channel.metrics.overall_score, 2), 'last_success': channel.metrics.last_success_time.isoformat() if channel.metrics.last_success_time else None, 'protocol_counts': channel.metrics.protocol_counts,},}
...
def main():
    try:
        config = ProxyConfig(); fetcher = ConfigFetcher(config); logger.info("Starting config fetching process...")
        categorized_configs = fetcher.fetch_all_configs(); total_config_count = sum(len(v) for v in categorized_configs.values())
        if total_config_count > 0:
            save_configs(categorized_configs, config); logger.info(f"Successfully processed {total_config_count} configs.")
            for protocol, configs in categorized_configs.items():
                if len(configs) > 0: logger.info(f"-> Found {len(configs)} {protocol.replace('://','')} configs")
        else: logger.warning("No valid configs found!")
        save_channel_stats(config); logger.info("Process finished.")
    except Exception as e: logger.error(f"Error in main execution: {str(e)}", exc_info=True)
